...
class Pervaporation:
    ...
    def calculate_partial_fluxes(
        ...
        inital_fluxes = numpy.matmul(
            permeances,
            self.mixture.get_nrtl_partial_pressures(feed_temperature, composition),
        )
    ...
    def ideal_diffusion_curve(
        ...
        diff_curv.partial_fluxes = [
            self.calculate_partial_fluxes(
                feed_temperature, permeate_temperature, composition, precision
            )
            for composition in composition_range
        ]
        diff_curv.separation_factor = [
            self.calculate_separation_factor(
                feed_temperature, permeate_temperature, composition, precision
            )
            for composition in composition_range
        ]
        diff_curv.total_flux = [
            numpy.sum(diff_curv.partial_fluxes[i])
            for i in range(len(diff_curv.partial_fluxes))
        ]
        diff_curv.PSI = numpy.multiply(
            numpy.subtract(
                diff_curv.separation_factor, numpy.ones(0, len(composition_range))
            ),
            diff_curv.total_flux,
        )
        if plot:
